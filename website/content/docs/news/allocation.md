...
So during search operation _Loselog_ creates and destroys `QString` and `QByteArray` 
...
_Loselog_ has been using the scalable memory allocator provided by the Intel
...
running and competing for memory allocation. On the other hand loselog uses
...
loselog executable is linked with the mimalloc-override.o file. This file must be provided first so
...
loselog use cases. Both indexing and search are around 10% faster.
...
# Overhead  Command          Shared Object                  Symbol
     ...
     4.53%  loselog_portable_  libc-2.32.so                   [.] 0x000000000015e01f
     4.43%  Thread (pooled)  libicuuc.so.68.2               [.] 0x00000000000e86d0
     3.47%  Thread (pooled)  loselog_portable                 [.] CompressedLinePositionStorage::at
...
memory allocator on MacOS. Loselog crashes with segmentation fault right at the start. So
...
stable than usual. Please report any bugs and crashes either on [GitHub](https://github.com/loseblue/loselog/issues)
