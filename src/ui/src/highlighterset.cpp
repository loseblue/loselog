...
bool Highlighter::matchLine( const QString& line, loselog::vector<HighlightedMatch>& matches ) const
{
    matches.clear();
...
{
    loselog::vector<RegularExpressionPattern> patterns(
                                                ...
                                                HighlightedMatchRanges& matches ) const
{
    ...
    loselog::vector<char> utf8Data( static_cast<size_t>( line.size() * 4 ) );
    ...
    for ( int index = static_cast<int>( highlighterList_.size() ) - 1; index >= 0; --index ) {
        ...
        loselog::vector<HighlightedMatch> thisMatches;
...
void HighlighterSetCollection::updateCombinedSet()
{
    ...
    for ( const HighlighterSet& set : loselog::as_const( highlighters_ ) ) {
