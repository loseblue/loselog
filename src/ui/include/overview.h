...
class Overview {
  public:
    ...
    const loselog::vector<WeightedLine>* getMatchLines() const;
    // Returns a list of lines (between 0 and 'height') representing marks.
    // (pointer returned is valid until next call to update*()
    const loselog::vector<WeightedLine>* getMarkLines() const;
  ...
  private:
    ...
    loselog::vector<WeightedLine> matchLines_;
    loselog::vector<WeightedLine> markLines_;
